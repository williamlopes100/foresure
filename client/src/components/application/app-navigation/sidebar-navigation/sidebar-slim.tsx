        ...
        >
            ...
            >
                <div className="flex justify-center px-3">
                    <UntitledLogoMinimal className="w-24" />
                </div>
                ...
                <div className="mt-auto flex flex-col gap-4 px-3 py-5">
                    ...
                    <AriaDialogTrigger>
                        ...
                        >
                            <Avatar status="online" initials={user ? `${(user.firstName?.[0] ?? '')}${(user.lastName?.[0] ?? '')}` : 'U'} size="md" alt={user ? `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() : 'User'} />
                        </AriaButton>
                        <AriaPopover
                        ...
                        >
                            <NavAccountMenu user={user} onSignOut={logout} />
                        </AriaPopover>
                    </AriaDialogTrigger>
                </div>
            </div>
                ...
                >
                    <div style={{ width: SECONDARY_SIDEBAR_WIDTH }} className="flex h-full flex-col px-4 pt-6">
                        ...
                        <div className="sticky bottom-0 mt-auto flex justify-between border-t border-secondary bg-primary px-2 py-5">
                            <div>
                                <p className="text-sm font-semibold text-primary">{user ? `${user.firstName ?? ''} ${user.lastName ?? ''}`.trim() || 'User' : 'User'}</p>
